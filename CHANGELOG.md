...
- **Detection ruleset single source of truth.** Unified the two parallel native-rule load pipelines in Fusion into one database-backed source. Native rules under `detections/` are now compiled on the fly by a dedicated native infix DSL compiler (`services/api/app/services/detections/native_match_when.py`) — replacing the old path that fed their infix `detection.condition` through the Sigma compiler, which errored every native rule — and routed into the `detection_rules` table via `seed_native_ruleset`. The legacy static-artifact pipeline (`scripts/export_detection_ruleset.py` → `services/fusion/app/data/detection_ruleset.json`, loaded once on boot) is decommissioned and no longer consulted. Fusion startup now awaits `reconcile_from_postgres` before the Kafka worker starts consuming, so no event lands against an empty rule map, and a Postgres-unreachable boot still fails fast rather than silently running zero rules.
- **Live hot-reload fixes.** (1) `broadcast_rule_reload` emits `match_when` at both root and under `compiled_rule`; `listen_detection_reload` tolerates both for forward/backward compat. (2) `DetectionEngine.apply_reload` now carries real `name`/`severity`/`category` metadata instead of the `{"name": rule_id, "severity": "medium", "category": "custom"}` fallback. (3) `reconcile_from_postgres` selects `name`/`severity`/`category` so metadata survives a Fusion restart. (4) `DELETE /api/v1/rules/{id}` broadcasts `DISABLE` so deleted rules are evicted from Fusion's memory immediately. All `rule_id` values entering the reload path are stringified. (5) The re-seed's `existing` branch refreshes `rule_body`, preserves operator `status`/hits/confidence, and recompiles on body change (in-memory MD5). (6) `test_detection_matcher_parity.py` no longer depends on the deleted JSON ruleset — it reads `scripts/detection_specs_index.all_specs()` instead.
- **Verification:** `test_native_match_when.py` (13), `test_sigma_match_when.py` (16), `test_detection_engine.py` (9), `test_detection_matcher_parity.py` (2) pass; `seed_native_detections --dry-run` reports 844 compiled / 25 `unsupported_stateful` / 0 error; no live reference to `detection_ruleset.json` remains.
