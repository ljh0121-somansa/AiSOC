...
def _all_rule_match_when() -> dict[str, dict]:
    """slug -> match_when from the canonical spec table (detection_specs_index)."""
    path = _REPO / "scripts" / "detection_specs_index.py"
    if not path.exists():
        return {}
    spec = importlib.util.spec_from_file_location("aisoc_detection_specs_index", path)
    assert spec and spec.loader
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return {st["slug"]: st["match_when"] for _, st in mod.all_specs()}


def _fixture_pairs() -> list[tuple[dict, dict]]:
    """Return (match_when, event) pairs from the canonical spec table keyed by
    ``slug`` (the deleted JSON ruleset can no longer be read); instead read the
    committed fixtures dir and look up each fixture's spec by name."""
    fixtures_dir = _REPO / "detections" / "fixtures"
    rules = _all_rule_match_when()
