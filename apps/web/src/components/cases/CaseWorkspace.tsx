      ...
      try {
        ...
        try {
          ...
          if (msg.type === 'agent.event') {
            ...
            if (kind === 'completed') {
              stopPolling();
              const isFailed = (msg.data as any)?.status === 'failed' || Boolean(msg.error);
              if (isFailed) {
                setInvestigationStatus('failed');
                setInvestigating(false);
                const errDetail = (msg.error as string) || summary || 'Investigation failed';
                ...
                toast.error(`Investigation failed: ${errDetail}`);
                closeWs();
                return;
              }
        ...
        try {
          ...
          if (inv.status === 'completed' || inv.status === 'failed') {
            ...
            if (inv.status === 'completed') {
              ...
              const errorDetail = inv.error || 'Unknown investigation error';
              setInvestigationData((prev) => ({
                ...(prev || {}),
                error: errorDetail,
              }));
              toast.error(`Investigation failed: ${errorDetail}`);
            }
          }
        } catch {
...
function InvestigationPanel({
  ...
  const hasPartialError = !!((data as any)?.error || recon?.error || forensic?.error || responder?.error);
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        {hasPartialError ? (
          <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-500/10 px-2.5 py-0.5 text-xs font-medium text-amber-300 ring-1 ring-amber-500/30">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        {Object.entries(nodesByType).map(([type, nodes]) => (
          ...
          >
            ...
            <ul className="space-y-1.5">
              {nodes.map((node) => {
                // 백엔드 properties 구조에 맞게 가장 적절한 이름을 추출
                const displayName =
                  (node.properties?.title as string) ||
                  (node.properties?.technique_id as string) ||
                  (node.properties?.alert_id as string) ||
                  node.id;
                ...
                return (
                  <li key={node.id} className="flex items-start gap-2 text-xs">
                    ...
                    <div className="min-w-0">
                      <p className="truncate font-mono text-slate-300" title={displayName}>
                        {displayName}
                      </p>
